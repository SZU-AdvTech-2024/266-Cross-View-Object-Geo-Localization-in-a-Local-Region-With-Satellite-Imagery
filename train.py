...
from model.AdvanceModel import AdvanceModel
from model.PFAModel import PFAModel
...
def main():
    ...
    parser.add_argument('--advance', dest='advance', default=False, action='store_true', help='advance')
    parser.add_argument('--pfa',dest='pfa', default=False, action='store_true', help='pfa')
    ...
    for batch_idx, (query_imgs, rs_imgs, mat_clickxy, ori_gt_bbox, _) in enumerate(train_loader):
        ...
        if batch_idx % args.print_freq == 0:
            print_str = 'Epoch: [{0}][{1}/{2}]\t' \
                ...
                loss=avg_losses, geo=avg_geo_losses, cls=avg_cls_losses, accu=avg_accu, miou=avg_iou,
...
def test_epoch(data_loader, model, args):
    ...
    for batch_idx, (query_imgs, rs_imgs, mat_clickxy, ori_gt_bbox, _) in enumerate(data_loader):
        ...
        accu_list, accu_center, iou, each_acc_list, _, _ = eval_iou_acc(pred_anchor, ori_gt_bbox, anchors_full,
                                                                        best_anchor_gi_gj[:, 1],
                                                                        best_anchor_gi_gj[:, 2], args.img_size,
